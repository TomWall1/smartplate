  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-parchment)' }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Add rule form */}
        <div className="rounded-[12px] border p-5 mb-6" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-stone)' }}>
          <h2 className="text-base font-bold mb-3" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>
            Add blocklist rule
          </h2>
          <p className="text-xs mb-3" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>
          ...
          <form onSubmit={addRule} className="flex flex-col gap-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  style={{ fontFamily: 'var(--font-ui)', borderColor: 'var(--color-stone)', color: 'var(--color-bark)' }}
                  required
                />
              </div>
              <div>
            ...
            <input
              ...
              style={{ fontFamily: 'var(--font-ui)', borderColor: 'var(--color-stone)', color: 'var(--color-bark)' }}
            />
            {addError && <p className="text-xs text-red-500" style={{ fontFamily: 'var(--font-ui)' }}>{addError}</p>}
        ...
        {error ? (
          ...
          <p className="text-center py-8" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>No blocklist rules yet.</p>
        ) : (
          <div className="flex flex-col gap-3">
            <p className="text-xs" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>{rules.length} rule{rules.length !== 1 ? 's' : ''}</p>
            {rules.map(rule => (
              <div key={rule.id} className="rounded-[16px] border p-4 flex items-start justify-between gap-3"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {rule.blocked_product_patterns?.length > 0 && (
                    <p className="text-xs mt-1" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>
                  ...
                  {rule.reason && (
                    <p className="text-xs mt-1 italic" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>{rule.reason}</p>
                  )}
                  <p className="text-xs mt-1" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>
