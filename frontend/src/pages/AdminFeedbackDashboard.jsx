      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-bark)' }}>Admin Dashboard</h1>
              <p className="text-xs" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>{user.email}</p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={process} disabled={processing}
              ...
              style={{ fontFamily: 'var(--font-ui)', background: 'var(--color-leaf)', color: '#fff' }}>
        ...
        {processMsg && (
          <div className="rounded-xl p-3 mb-4 text-sm" style={{ background: '#DCE4D6', color: 'var(--color-bark)', fontFamily: 'var(--font-ui)' }}>
            {processMsg}
          </div>
        )}

        {error ? (
          <div className="rounded-xl p-4 text-center" style={{ background: '#fee2e2', color: '#b91c1c', fontFamily: 'var(--font-ui)' }}>{error}</div>
        ) : loading ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            {summary.length > 0 && (
              <div>
                <h2 className="text-base font-bold mb-3" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>
                      ...
                      <tr style={{ background: 'var(--color-mist)' }}>
                        {['Ingredient', 'Product', 'Incorrect', 'Correct'].map(h => (
                          <th key={h} className="text-left px-4 py-2 text-xs font-bold"
                            style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {summary.map((s, i) => (
                        <tr key={i} style={{ borderTop: '1px solid var(--color-stone)', background: 'var(--color-surface)' }}>
                          <td className="px-4 py-2 text-xs" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>{s.ingredient_name}</td>
                          <td className="px-4 py-2 text-xs" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>{s.product_name}</td>
                          <td className="px-4 py-2 text-xs font-bold" style={{ fontFamily: 'var(--font-ui)', color: '#b91c1c' }}>{s.incorrect}</td>
                          <td className="px-4 py-2 text-xs font-bold" style={{ fontFamily: 'var(--font-ui)', color: '#15803d' }}>{s.correct}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div>
              ...
              {feedback.length === 0 ? (
                <p className="text-center py-6" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>
                ...
                <div className="rounded-[12px] border overflow-hidden" style={{ borderColor: 'var(--color-stone)' }}>
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {feedback.map(f => (
                        <tr key={f.id} style={{ borderTop: '1px solid var(--color-stone)', background: 'var(--color-surface)' }}>
                          <td className="px-3 py-2 text-xs whitespace-nowrap" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>
                            {new Date(f.created_at).toLocaleDateString()}
                          </td>
                          <td className="px-3 py-2 text-xs max-w-[120px] truncate" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>
                            {f.recipe_title || '—'}
                          </td>
                          <td className="px-3 py-2 text-xs" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>{f.ingredient_name}</td>
                          <td className="px-3 py-2 text-xs" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-bark)' }}>{f.product_name}</td>
                          <td className="px-3 py-2 text-xs">
                            <span className="px-2 py-0.5 rounded-full text-xs font-bold"
                              style={{ background: f.feedback_type === 'incorrect' ? '#fee2e2' : '#DCE4D6', color: f.feedback_type === 'incorrect' ? '#b91c1c' : '#15803d', fontFamily: 'var(--font-ui)' }}>
                              {f.feedback_type}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-xs max-w-[200px] truncate" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-muted)' }}>
                            {f.reason || '—'}
                          </td>
                        </tr>
                      ))}
